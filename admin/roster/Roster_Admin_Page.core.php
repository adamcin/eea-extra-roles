 ...
 *
 *
 * @package     Roster_Admin_Page
 * @subpackage  admin/Roster_Admin_Page.core.php
 * @author      Darren Ethier
 *
 * ------------------------------------------------------------------------
 */
class Roster_Admin_Page extends EE_Admin_Page
{
    ...
    {
        wp_register_script('ee-calendar-admin-js', EE_CALENDAR_ADMIN_ASSETS_URL . 'calendar-admin.js',
            array('jquery', 'wp-color-picker'), EE_CALENDAR_VERSION, true);
    ...
    {
        $this->_template_args['calendar_config'] = EE_Config::instance()->get_config('addons', 'EE_Calendar',
            'EE_Calendar_Config');
        ...
        $this->_template_args['reset_url'] = EE_Admin_Page::add_query_args_and_nonce(array(
            'action' => 'reset_settings',
            'return_action' => $this->_req_action
        ), EE_CALENDAR_ADMIN_URL);
        $this->_set_add_edit_form_tags('update_settings');
        $this->_set_publish_post_box_vars(null, false, false, null, false);
        $this->_template_args['admin_page_content'] = EEH_Template::display_template(EE_CALENDAR_ADMIN_TEMPLATE_PATH . $template,
    ...
    protected function _usage()
    {
        $this->_template_args['admin_page_content'] = EEH_Template::display_template(
            ...
            apply_filters(
                'FHEE__Roster_Admin_Page___usage__calendar_usage_info__template_args',
                array()
            ),
            true
        );
            ...
            foreach ($calendar_req_data as $top_level_key => $top_level_value) {
                if (is_array($top_level_value)) {
                    foreach ($top_level_value as $second_level_key => $second_level_value) {
                        if (property_exists($config, $top_level_key) && property_exists($config->$top_level_key,
                                $second_level_key)
                            && $second_level_value != $config->$top_level_key->$second_level_key) {
                            $config->$top_level_key->$second_level_key = $this->_sanitize_config_input($top_level_key,
...
//      echo "$top_level_key [$second_level_key] with value $value will be sanitized as a $sanitization_method<br>";
        switch ($sanitization_method) {
            ...
            default:
                ...
                EE_Error::add_error(sprintf(__("Could not sanitize input '%s' because it has no entry in our sanitization methods array",
                    "event_espresso"), $input_name), __FILE__, __FUNCTION__, __LINE__);
                return null;
        }
    }
}
